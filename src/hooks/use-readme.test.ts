...
describe('useReadme', () => {
  it('fetches nothing while disabled', () => {
    ...
    expect(result.current).toEqual({ readme: null, loading: false })
  })
  ...
  it('loads once enabled, reporting loading until the README lands', async () => {
    ...
    expect(result.current).toEqual({ readme: null, loading: true })

    await act(async () => {
      pending.resolve(README)
    })

    expect(result.current).toEqual({ readme: README, loading: false })
  })
