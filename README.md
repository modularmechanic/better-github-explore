...
Runs entirely in the browser: no backend, and no token needed for the public tabs — a personal access token unlocks the personalised You tab.
...
Unauthenticated GitHub allows 60 core + 10 search requests/hour. Every response is cached in
...
5000 requests/hour and unlock the **You** tab. It is stored in this browser only and sent only to
`api.github.com`.
